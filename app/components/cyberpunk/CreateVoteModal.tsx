...
export function CreateVoteModal({ isOpen, onClose, onVoteCreated }: CreateVoteModalProps) {
  ...
  const calculateEndTime = () => {
    ...
    if (formData.duration === 'custom') {
      ...
      const option = DURATION_OPTIONS.find(opt => opt.value === formData.duration);
  ...
  const handleCreateVote = async () => {
    ...
    try {
      ...
      console.log("formData.selectedToken",formData.selectedToken, isAddress(formData.selectedToken));
      // Step 2: Store in database
      const voteData = {
        vote_pubkey: txResult.votePubkey!,
        token_address: formData.selectedToken === 'SOL' ? null : formData.selectedToken,
      ...
      if (formData.initialVote && result.data.id) {
        const voteResult = await castVote(txResult.votePubkey!, formData.initialVote);
  ...
  return (
    ...
    >
      <div className="space-y-6">
          ...
          {[1, 2, 3].map((step) => (
            <div key={step} className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                step <= currentStep ? 'bg-cyber-green text-black' : 'bg-cyber-dark border border-cyber-cyan cyber-cyan'
              }`}>
                {step}
              </div>
              {step < 3 && (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {formData.tags.map(tag => (
                      <span key={tag} className="bg-cyber-green text-black px-2 py-1 text-xs font-bold">
                        {tag}
                        <button 
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {DURATION_OPTIONS.map(option => (
                      <button
                        key={option.value}
                        onClick={() => setFormData(prev => ({ ...prev, duration: option.value as 'custom' | '1h' | '1d' | '1w' }))}
                  ...
                  {formData.duration === 'custom' && (
                    <div className="flex gap-2 mt-2">
                      ...
                      <select
                        ...
                        onChange={(e) => setFormData(prev => ({ 
                          ...prev, 
                          customUnit: e.target.value as 'minutes' | 'hours' | 'days'
                        }))}
                ...
                <div>
                  ...
                  {loadingTokens ? (
                    <div className="flex items-center justify-center p-4">
                      ...
                      <span className="cyber-cyan text-sm">LOADING_TOKENS...</span>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <button
                        onClick={() => setFormData(prev => ({ ...prev, selectedToken: 'SOL' }))}
                      ...
                      {availableTokens.map(token => (
                        <button
                          key={token.mint}
                          onClick={() => setFormData(prev => ({ ...prev, selectedToken: token.mint }))}
                          className={`w-full p-3 text-left cyber-font text-sm ${
                        ...
                        >
                          <div className="flex justify-between items-center">
                            ...
                            <span>{token.balance.toFixed(token.decimals > 6 ? 2 : token.decimals)}</span>
                          </div>
                        </button>
                      ))}
                    </div>
        ...
        {currentStep === 3 && (
          <div className="space-y-4">
            <Terminal header="STEP_3: CONFIRMATION_AND_DEPLOY.exe" glowColor="pink">
              <div className="p-4 space-y-4">
                <div className="bg-cyber-darker p-3 border border-cyber-cyan">
                  <h4 className="cyber-cyan font-bold mb-2">VOTE_SUMMARY:</h4>
                  <div className="space-y-1 text-sm">
                    <div><span className="cyber-yellow">TITLE:</span> {formData.title}</div>
                    <div><span className="cyber-yellow">CATEGORY:</span> {formData.category.toUpperCase()}</div>
                    <div><span className="cyber-yellow">DURATION:</span> {
                      formData.duration === 'custom' 
                        ? `${formData.customDuration} ${formData.customUnit.toUpperCase()}`
                        : DURATION_OPTIONS.find(opt => opt.value === formData.duration)?.label
                    }</div>
                    <div><span className="cyber-yellow">TOKEN:</span> {
                      formData.selectedToken === 'SOL' 
                        ? 'SOL' 
                        : availableTokens.find(t => t.mint === formData.selectedToken)?.symbol || 'Unknown'
                    }</div>
                    <div><span className="cyber-yellow">END_TIME:</span> {calculateEndTime().toLocaleString()}</div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <button
                      onClick={() => setFormData(prev => ({ ...prev, initialVote: undefined }))}
                      className={`p-2 text-sm font-bold cyber-font ${
                    ...
                    <button
                      onClick={() => setFormData(prev => ({ ...prev, initialVote: 'yes' }))}
                      className={`p-2 text-sm font-bold cyber-font ${
                    ...
                    <button
                      onClick={() => setFormData(prev => ({ ...prev, initialVote: 'no' }))}
                      className={`p-2 text-sm font-bold cyber-font ${
